...
class ContourView(ttk.Frame):
    ...
    def _setup_ui(self):
        ...
        self.coordinates_label = ttk.Label(self.status_frame, textvariable=self.coordinates_var)
        self.coordinates_label.pack(side=tk.RIGHT, padx=5)
    ...
    def _setup_axes_style(self):
        """Set up axes style and limits."""
        self.axes.set_xlabel('X Coordinate (m)')
        self.axes.set_ylabel('Y Coordinate (m)')
        self.axes.set_title('Terrain Contour Map - Tunnel Planning')
        self.axes.grid(True, alpha=0.3)
    ...
    def _on_mouse_motion(self, event):
        ...
        x, y = event.xdata, event.ydata
        self.coordinates_var.set(f"X: {x:.1f}, Y: {y:.1f}")

        # Handle hover effects
        if self.visualization_state.interaction_mode == InteractionMode.TUNNEL_EDITING:
    ...
    def _on_key_press(self, event):
        ...
        if event.key == 'escape':
            ...
            self.status_var.set("Point selection mode")

        elif event.key == 'e':
            # Edit mode
            self.visualization_state.set_interaction_mode(InteractionMode.TUNNEL_EDITING)
            self.status_var.set("Tunnel edit mode")
    ...
    def export_view(self, filename: str):
        ...
        self.status_var.set(f"View exported to {filename}")
